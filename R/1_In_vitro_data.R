#-----------------------------------------------------------------------------------#
# APCRA case study: generate master toxcast and astar bioactivity data file
# 
...
tcplConf(user='_', pass='', db='invitrodb_v3', drvr='MySQL', host='')
...
mc7 <- dbGetQuery(con, "SELECT 
                  mc7.*
                  FROM
                  prod_internal_invitrodb_v3.mc7;") %>% 
  data.table() 
